                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($user->remaining_days < 0)
                                                    <span class="badge bg-secondary">Bitmiş</span>
                                                    @else
                                                    {{ $user->remaining_days }} gün
                                                    @endif
                                            </td>
                                            <td>
                                                @if($user->status_class == 'danger')
                                                <span class="badge bg-danger">{{ $user->status_text }}</span>
                                                @elseif($user->status_class == 'warning')
                                                <span class="badge bg-warning text-dark">{{ $user->status_text }}</span>
                                                @else
                                                <span class="badge bg-info">{{ $user->status_text }}</span>
                                                @endif
                                            </td>
                                        </tr>
                                        @empty
                                        <tr>
                                            <td colspan="5" class="text-center">Müddəti bitmək üzrə olan müqavilə yoxdur</td>
                                        </tr>
                                        @endforelse
                                    </tbody>
                                </table>
                    ...
                    <div class="card-body">
                        ...
                        <div class="activity">
                            <div class="activity-item d-flex">
                                <div class="activite-label">{{ $allUsersCount }}</div>
                                <i class="bi bi-circle-fill activity-badge text-success align-self-start"></i>
                                <div class="activity-content">Ümumi işçi sayı</div>
                            </div>

                            @foreach($Organizations as $index => $org)
                            @php
                            $badgeColors = ['text-primary', 'text-danger', 'text-warning', 'text-info', 'text-secondary', 'text-dark'];
                            $color = $badgeColors[$index % count($badgeColors)];
                            @endphp
                            <div class="activity-item d-flex">
                                <div class="activite-label">{{ $org->user_count }}</div>
                                <i class="bi bi-circle-fill activity-badge {{ $color }} align-self-start"></i>
                                <div class="activity-content">{{ $org->name }}</div>
                            </div>
                            @endforeach
                        </div>
                    </div>
